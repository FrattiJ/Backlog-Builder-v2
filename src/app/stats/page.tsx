...
import { getAllEntries, getAllSessions, ENTRIES_CHANGED_EVENT } from '@/lib/db'
...
export default function StatsPage() {
  ...
  useEffect(() => {
    const load = () => Promise.all([getAllEntries(), getAllSessions()])
  ...
  const tooltipStyle = { contentStyle: { background: 'var(--bg-card)', border: '1px solid var(--border-dim)', borderRadius: 0, color: 'var(--text-hi)', fontFamily: 'var(--font-mono)', fontSize: 11 } }

  return (
    <div style={{ padding: 32, maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        <p style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-dim)', letterSpacing: '0.2em', marginBottom: 4 }}>SYSTEM / ANALYTICS</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
        <div style={{ padding: '1px', clipPath: CLIP, background: '#7c3aed55' }}>
          <div style={{ background: 'var(--bg-card)', clipPath: CLIP, width: '100%', padding: 20, position: 'relative' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: 'var(--text-hi)', margin: 0, marginBottom: 16, letterSpacing: '0.1em' }}>ENTRIES BY CATEGORY</h2>
        ...
        <div style={{ padding: '1px', clipPath: CLIP, background: '#0891b255' }}>
          <div style={{ background: 'var(--bg-card)', clipPath: CLIP, width: '100%', padding: 20, position: 'relative' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: 'var(--text-hi)', margin: 0, marginBottom: 16, letterSpacing: '0.1em' }}>STATUS DISTRIBUTION</h2>
            {statusDist.length === 0 ? (
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-mute)', textAlign: 'center', padding: 24, margin: 0 }}>NO DATA YET</p>
            ) : (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {statusDist.map((d) => (
                    <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-dim)', flex: 1 }}>{d.name}</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-hi)', fontWeight: 700 }}>{d.value}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <div style={{ padding: '1px', clipPath: CLIP, background: '#d9770655' }}>
          <div style={{ background: 'var(--bg-card)', clipPath: CLIP, width: '100%', padding: 20, position: 'relative' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: 'var(--text-hi)', margin: 0, marginBottom: 16, letterSpacing: '0.1em' }}>RATING DISTRIBUTION</h2>
        ...
        <div style={{ padding: '1px', clipPath: CLIP, background: '#22c55e55' }}>
          <div style={{ background: 'var(--bg-card)', clipPath: CLIP, width: '100%', padding: 20, position: 'relative' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: 'var(--text-hi)', margin: 0, marginBottom: 16, letterSpacing: '0.1em' }}>TOP RATED</h2>
            {topRated.length === 0 ? (
                ...
                {topRated.map((e, i) => {
                  ...
                  return (
                    <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, fontWeight: 700, color: 'var(--text-dim)', width: 20, textAlign: 'center' }}>#{i + 1}</span>
                      {e.cover_url && <img src={e.cover_url} alt="" style={{ width: 28, height: 38, objectFit: 'cover', flexShrink: 0, border: `1px solid color-mix(in srgb, var(--text-dim) 20%, transparent)` }} />}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontFamily: 'var(--font-display)', fontSize: 14, color: 'var(--text-hi)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.title}</p>
                        <p style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-dim)', margin: 0, marginTop: 2 }}>{hobby?.label}</p>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                        <span style={{ color: '#d97706', fontSize: 12 }}>★</span>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-hi)', fontWeight: 700 }}>{e.rating}</span>
                      </div>
                    </div>
                  )
                })}
